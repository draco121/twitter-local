...
router.get('/listfollowers', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const followers = await networkService.getFollowers(userinfo.data);
    res.json(followers);
  } catch (err) {
    logger.error(err);
    res.status(500).send(err.message);
...
router.get('/listfollowings', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const following= await networkService.getFollowing(userinfo.data);
    res.json(following);
  } catch (err) {
    logger.error(err);
    res.status(500).send(err.message);
...
router.get('/listsentrequest', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const sentRequests = await networkService.getSentRequests(userinfo.data);
...
router.get('/listrecivedrequest', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const recivedRequests = await networkService.
        getRecivedRequests(userinfo.data);
...
router.get('/followerscount', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const followerscount = await networkService.
        getFollowersCount(userinfo.data);
...
router.get('/followingcount', async (req, res)=>{
  try {
    const token = req.headers.authorization;
    const userinfo = jwtUtils.validateToken(token);
    const followingcount = await networkService.
        getFollowingCount(userinfo.data);
